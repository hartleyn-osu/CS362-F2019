...
int checkHandleBaronCardEffect(int choice1, int handPos, int currentPlayer, struct gameState *post) {
    struct gameState pre;
    memcpy (&pre, post, sizeof(struct gameState));

    int r;

    r = handleBaronCardEffect(choice1, handPos, currentPlayer, post);
...
int main () {
    ...
    printf ("Testing handleBaronCardEffect.\n");
    ...
    for (n = 0; n < 2000; n++) {
        ...
        assert(checkHandleBaronCardEffect(choice1, handPos, G.whoseTurn, &G) == 0);
